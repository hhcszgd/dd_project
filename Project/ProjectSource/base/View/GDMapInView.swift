...
//        }
    ...
    func setupMapView()  {
        ...
        self.mapView.frame = CGRect(x: 0, y: 100, width: self.bounds.size.width, height: self.bounds.size.height - 49 - 100)
...
//        request.requestsAlternateRoutes = true
        GDLocationManager.share.geoCoder.geocodeAddressString("郑州") { (sourceCLPlacemarks, error) in
            ...
            GDLocationManager.share.geoCoder.geocodeAddressString("上海") { (destinationCLPlacemarks, error) in
                ...
                self.getRoutes(sourceCoordinate: sourceMKPlacemark.coordinate, destinationCoordinate: destinationMKPlacemark.coordinate, complate: { (respons ) in
                    if #available(iOS 10.0, *) {
                        let start =  CLLocationCoordinate2D.init(latitude:34.44 , longitude:113.42 )
                        let end = CLLocationCoordinate2D.init(latitude:39.54 , longitude:116.23 )
                        self.getRoutes(sourceCoordinate: start, destinationCoordinate: end, complate: { (resp) in
                            self.mapView.addOverlays([respons!.routes.first!.polyline , resp!.routes.first!.polyline])
    ...
    func drawNaviLine()  {
        ...
        GDLocationManager.share.geoCoder.geocodeAddressString("北京") { (sourceCLPlacemarks, error) in
            ...
            GDLocationManager.share.geoCoder.geocodeAddressString("上海") { (destinationCLPlacemarks, error) in
                ...
                ///:test begain
                let middle =  MKCircle.init(center: CLLocationCoordinate2D.init(latitude: 34.44, longitude:113.42 ), radius: 10)
                self.mapView.add(middle)
                ///:test end
    ...
    func settheUserLocationToScreenCenter(location: MKUserLocation)  {
        ...
        let span :  MKCoordinateSpan = MKCoordinateSpan.init(latitudeDelta: 11.100001, longitudeDelta: 11.100001)//初始显示范围,值越小越精确
