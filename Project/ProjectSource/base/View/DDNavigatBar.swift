...
class DDNavigatBar: UIView {
    ...
    var leftBarButtons  = [UIView]() {
        ...
        didSet{
            ...
            if currentType == .withBackBtn {
                ...
                leftSubViewsContainer.frame = CGRect(x: 44, y: NavigationBarHeight - 44, width: leftSubViewsContainerW, height: 44)
                for (index , newSub) in leftBarButtons.enumerated() {
                    if 44.0  * CGFloat(index+1)  > UIScreen.main.bounds.size.width * 0.5 - 44.0 {
                        leftSubViewsContainer.frame = CGRect(x: 44, y: NavigationBarHeight - 44, width: 44.0  * CGFloat(index), height: 44)
                        return
                    }
                ...
                leftSubViewsContainer.frame = CGRect(x: 0, y: NavigationBarHeight - 44 , width: leftSubViewsContainerW, height: 44)
                for (index , newSub) in leftBarButtons.enumerated() {
                    if 44.0  * CGFloat(index+1)  > UIScreen.main.bounds.size.width * 0.5  {
                        leftSubViewsContainer.frame = CGRect(x: 0, y: NavigationBarHeight - 44, width: 44.0  * CGFloat(index), height: 44)
                        return
                    }
    ...
    var rightBarButtons  = [UIView]() {
        ...
        didSet{
            ...
            rightSubViewsContainer.frame = CGRect(x: UIScreen.main.bounds.size.width - rightSubViewsContainerW, y: NavigationBarHeight - 44, width: rightSubViewsContainerW, height: 44)
            for (index , newSub) in rightBarButtons.enumerated() {
                if 44.0  * CGFloat(index + 1 )  > UIScreen.main.bounds.size.width * 0.5  {
                    rightSubViewsContainer.frame = CGRect(x: UIScreen.main.bounds.size.width - 44.0 * CGFloat(index), y: NavigationBarHeight - 44, width: 44.0  * CGFloat(index), height: 44)
                    return
                }
    ...
    var navTitleView   =  NavTitleView(){
        ...
        didSet {
            ...
            var x : CGFloat = 0.0
            let y : CGFloat = NavigationBarHeight - 44 + inset.top
            var w : CGFloat = 0.0
    ...
    */
    override init(frame: CGRect) {
        ...
        backBtn = UIButton(frame: CGRect(x: 0, y: NavigationBarHeight - 44, width: 44, height: 44))
        titleLabel = UILabel(frame: CGRect(x: 0, y: NavigationBarHeight - 44 , width: UIScreen.main.bounds.size.width - 88.0, height: 44))
        titleLabel.isHidden = true;
        titleLabel.textAlignment = NSTextAlignment.center
        self.addSubview(titleLabel)
        leftSubViewsContainer  = UIView()
        rightSubViewsContainer = UIView()
        titleLabel.center = CGPoint(x: UIScreen.main.bounds.size.width/2.0, y: NavigationBarHeight - 44 + 22  );
        leftSubViewsContainer.backgroundColor = UIColor.randomColor()
        rightSubViewsContainer.backgroundColor = UIColor.randomColor()
...
//                mylog("上拉加载部分")
    ...
    let originFrame = CGRect(x: 0, y: 0, width: SCREENWIDTH, height: NavigationBarHeight)
    let targetFrame = CGRect(x: 0, y: -NavigationBarHeight, width: SCREENHEIGHT, height: NavigationBarHeight)
