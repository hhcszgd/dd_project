...
//MARK:////////////////////////////////////////////更改语言相关//////////////////////////////////////////////////////
    ...
    class  func performChangeLanguage(targetLanguage : String)  {
        ...
        if targetLanguage == LanguageTableName && targetLanguage == LFollowSystemLanguage {//切换前后语言相同 , 除了提示之外不做任何变化
            ...
            GDAlertView.alert("\(noticStr_currentLanguageIs)\(GDLanguageManager.text(LLanguageID) )", image: nil, time: 2, complateBlock: nil)
            
        }else if (targetLanguage == LFollowSystemLanguage){//由自定义语言切换为跟随系统语言
            if GDLanguageManager.text(LLanguageID)  ==  GDLanguageManager.gotcurrentSystemLanguage() {//当切换前的自定义语言跟当前系统语言一样时 ,只保存 ,不重新切换 (如何获取系统语言??)
            ...
            if GDLanguageManager.text(LLanguageID) == "languageID"  {//gotTitleStr(key:)这个方法如果找不到键所对应的值 , 就把键返回,同时也说明本地没有相应的语言包
...
// 获取当前设备支持语言数组
    ...
    class  func gotcurrentSystemLanguage() -> String{
        let languages = GDStorgeManager.standard.object(forKey: "AppleLanguages")//同NSLocale.preferredLanguages
